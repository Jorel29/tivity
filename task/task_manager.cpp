...
//      - Find out if calling member pm_filename is valid
...
public:
  ...
  void new_task(std::vector<Task> &t_list, unsigned int &curr_id) {
    ...
    Writer::write(new_task, pm_filename);
  ...
  void edit_status(Task &task) {
    ...
    while (true) {
      ...
      std::cout << "Mark Complete(c)   Mark Incomplete(i) Done Marking(d)"
                << std::endl;
      std::cin >> input;
      if (input == 'd')
        break;
  ...
  void manage_tasks(std::vector<Task> &t_list) {
    ...
    while (std::cin.fail()) {
      std::cin.clear();
      std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      std::cout << "Invalid input, input a proper id (positive integer)"
                << std::endl;
      std::cin >> taskid;
    }
    ...
    while (true) {
      ...
      std::cout << "Task Status (s)   Set Priority (p)   Edit Desc (d)   "
                   "Remove Task (r)  Exit Management(e)"
                << std::endl;
      std::cin >> input;
      if (input == 'e')
        break;
      ...
      if (input == 'r') {
        ...
        if (input == 'y') {
          ...
          Writer::full_write(t_list, pm_filename);
          break;
        } else {
          continue;
        }
